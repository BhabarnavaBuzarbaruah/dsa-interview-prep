...
// Approach: Sort the array, then fix one number and use two-pointer technique.
// Function to find all unique triplets that sum to zero
    public static List<List<Integer>> threeSum(int[] nums) {
        List<List<Integer>> result = new ArrayList<>();

        // Sort the array to make it easier to skip duplicates and use two-pointer approach
        Arrays.sort(nums);
